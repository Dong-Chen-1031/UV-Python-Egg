# Python with UV - Yolks 映像與 Pterodactyl/Pelican Eggs

一個完整的解決方案，用於在 Pterodactyl/Pelican 面板上運行 Python 應用程式，具備現代化工具和自動化工作流程，包含：

1. **運行時 Docker 映像 (Yolks)**：預裝 UV 套件管理器的 Python 映像（3.8-3.14），提供極速的依賴管理
2. **安裝器 Docker 映像**：具有 root 權限和所有編譯依賴預裝的專用建置環境
3. **Pterodactyl/Pelican Eggs**：基於 [pelican-eggs/generic](https://github.com/pelican-eggs/generic) 的即用型 egg 配置，支援 UV 和傳統 pip 工作流程
4. **自動化 Egg 生成**：智慧型 hen.py 生成器，自動從模板維護 egg 配置

此專案提供了一個現代化、高效能的 Python 開發環境，具備自動依賴管理、零權限問題，以及完全自動化的 CI/CD 工作流程。
...
**匯入步驟：**
1. 在您的管理面板中，導航至 **Nests** → 選擇您的 nest → **Import Egg**
2. 在匯入欄位中貼上上方適當的 URL
3. 點擊匯入，egg 將自動配置完成
...
匯入 egg 後：
1. 使用「Python with UV」egg 建立新伺服器
2. 從 Docker 映像下拉選單中選擇所需的 Python 版本
3. 配置您的應用程式：
   - **Git Repo Address**：您的儲存庫 URL
   - **App py file**：您的主要 Python 檔案（預設：`app.py`）
   - **Requirements file**：您的 requirements 檔案（預設：`requirements.txt`）
4. 啟動您的伺服器，應用程式將自動部署
